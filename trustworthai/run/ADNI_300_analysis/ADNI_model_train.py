# copy of the jupyter notebook script I have becaues the model is taking way way way too long to train.
# trainer
print("strawberry")
...
def construct_parser():
    ...
    # folder arguments
    parser.add_argument('--target', default="WMH_Deep", type=str)
    parser.add_argument('--split', default=0, type=int)
    parser.add_argument('--input_type', default="flair", type=str)
    parser.add_argument('--model_name', default="SSN_Ens", type=str)
...
def main(args):
    ...
    out_classes = 7 if target == "Total" else 4 # 4 for dwmh and pvwmh
    ...
    imgs_ds = ImgsAndDfDataset(imgs_ds, combined_df, selected_fields=fields, label_field=target)
    ...
    for i in range(7):
        ...
        model_raw = PredictionModel(image_channels=image_channels, num_clin_features=clin_features, out_classes=out_classes, channel_size_divide=8)#.cuda()
        loss = xent_wrapper()
        ...
        # train the model
        trainer = get_trainer(max_epochs, f"/disk/scratch_big/s2208943/{target}_{input_type}_{split}/", early_stop_patience=early_stop_patience)
        ...
        #torch.save(model, f"/home/s2208943/ipdis/results/ADNI_Fazekas_cv/{target}_{input_type}_{split}_{i}.ckpt")
        ...
        out_file = f"/home/s2208943/ipdis/UQ_WMH_methods/trustworthai/run/ADNI_300_analysis/{model_name}_{target}_{input_type}_{split}.txt"
